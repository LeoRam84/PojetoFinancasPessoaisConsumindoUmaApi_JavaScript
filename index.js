...
async function saveTransaction(ev) {
  ...
  ev.target.reset()
  await setup(); // Renderiza todas as transações novamente após editar ou adicionar uma nova
  // updateBalance()
}
...
async function setup() {
  ...
  transactionsContainer.innerHTML = ''; // Limpa o conteúdo antes de renderizar novamente
  transactions.forEach(renderTransaction);
  updateBalance();
}
